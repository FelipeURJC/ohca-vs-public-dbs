function [output] = feature_selection(X,y,id,method, params, metric, FeatNames)
%
...
%
%   params: struct
%       If method == 'BST' => tree parameters. Fields: method, lr (learning rate), iterations (iter), minleaf
%       If method == 'LLR' => optimal lambda. Fields: lambda_opt
%
...
%      · feat_index: index of the selected features
...
%      · bootstrap_metrics: all the error metrics for each resample and
%        feature set size
%      · sortedIndex: sorted index of the features for each resample.
%
%                                            Carlos Figuera, 2015.


%% (0) Set the free parameters for the tree
if strcmp(method, 'BST'),
    tree_method  = params.method;
...
for b=1:B
    ...
    switch method,
        ...
        case 'LLR',
            ...
            [~, idx] = min(abs(modelo.lambda - params.lambda_opt));
            betas = modelo.beta(:,idx);
            imp(b,:) = abs(betas);
    end
    
    [~, sortedIndex(b,:)] = sort(imp(b,:));
    
    %%% 3. Compute error and ber for different number of features    
    for n=1:Nfeat
        idx = sortedIndex(b,n:Nfeat);
